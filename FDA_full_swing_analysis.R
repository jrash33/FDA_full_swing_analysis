...
badswingFUN=function(listX){
  ...
  if(typeof(col1)=='character'){difflogical=TRUE}
  else{
...
impactFUN=function(listX){
  ...
  #new addition 3/29/19
  #th
  if (abs(maxloc-nrow(V_ImpactLocation))<300){
    len=nrow(V_ImpactLocation)
    dist=abs(maxloc-nrow(V_ImpactLocation))
    V_ImpactLocation=V_ImpactLocation[-(len-(dist+50)):-len,]
    maxloc=which.max(matrix(unlist(V_ImpactLocation),nrow = nrow(V_ImpactLocation), ncol = length(V_ImpactLocation)))
  }
  #end of new addition
  
  #new addition 10/29/18
  if (length(which(is.na(V_ImpactLocation[(maxloc-30):maxloc])))>25){
    #new addition 3/29/19: delete all numbers leading up to the max as well just to be safe
    while (is.na(V_ImpactLocation[maxloc-1])==FALSE){
      maxloc=maxloc-1
    }#end new addition
...
registerFUN=function(AllSwingsMat,SizeVec,MatrixSize,val){
  ...
  RegisteredSwings=mapply(sumZeroFUN,Matrix=RegisteredSwings,OffsetVal=OffsetVal,cutval=1100,SIMPLIFY=FALSE)
...
clubanalysisFUNavg=function(List,Size,clubs,clubofinterest1,clubofinterest2){
  ...
  }else if(length(club2)>length(club1)){
...
directory = paste(current_wd, "/swing_data", sep = "")
setwd(directory)
...
if (length(badswings)!=0){
MoCapALL=MoCapALL[-badswings]
temp=temp[-badswings]}

#hardcode delete bad swings (fix later w automation): finds after running polyfitspeed later (rerun)
# badswings=c(100,108,275,358)
...
temp3=gsub("X026_DEEP","X026_RWD_",temp3)
temp3=gsub("X026_SHAL","X026_FWD_",temp3)
...
plot(SwingXdir_smoothed[[1]],col="red")
lines(AVGSwingXdir[[1]],col="blue",lwd=3)
...
marker2a=V_HoselCenter
marker2b=V_HoselCenterGR
Time2=Time
...
deflection=mapply(function(marker1,marker2){marker1-marker2},marker1=marker2a,marker2=marker2b)
...
ANALYSISspeed=clubanalysisFUNavg(deflectionX,Size,clubs,2,1)
ANALYSISshaftspeed=clubanalysisFUNavg(ShaftSpeedMAG,Sizexs,clubs,2,1)
...
#SORTED BY PLAYER AND CLUB
ANALYSISspeed=clubanalysisFUNavg(VTot,SizeS,clubs,1,2)
...
#Velocity
plot(Velocity_smoothed[[1]],xlim=c(980,1000),ylim=c(50,55))
